...
namespace Apps.Repository
{
    public abstract class BaseRepository<T> : IBaseRepository<T> where T:class
    {
        ...
        /// <param name="where">查询条件</param>
        /// <returns></returns>
        public virtual IQueryable<T> GetList(Expression<Func<T, bool>> where=null, Expression<Func<T, bool>> query=null)
        {
        ...
        /// <returns></returns>
        public virtual IQueryable<T> GetListSort<TKey>(Expression<Func<T, bool>> where, Expression<Func<T, TKey>> orderBy, bool isAsc = true)
        {
            if (isAsc)
            {
        ...
        #region 4.1分页查询 IQueryable<T> GetListPaged<S>(int pageSize, int pageIndex, out int total, Expression<Func<T, bool>> whereLambda, bool isAsc, Expression<Func<T, bool>> orderByLambda)
            ...
            , Expression<Func<T, bool>> whereLambda, bool isAsc, Expression<Func<T,S>> orderByLambda)
        {
            ...
            if (whereLambda != null)
            {
                queryable = dbSet.Where(whereLambda);
            }
            else {
                queryable = dbSet;
            }
