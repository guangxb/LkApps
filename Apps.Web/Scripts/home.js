...
function loadAccordionMenu(parentId) {
	...
	function (xhrObj) {
		if (msgProcess(xhrObj)==false) return;
		...
		var fristTitle;
		//if (xhrObj.Status != "1") {
		//    window.location = xhrObj.BackUrl;
		//}
		$.each(data, function (i, e) { //循环创建手风琴的项
			...
			$.post("/" + _YMGlobal.Config.currentCulture + "/Home/GetTreeByEasyui?id=" + id, function (xhrObj) { //循环创建树的项
				if (msgProcess(xhrObj) == false) return;
...
function loadTreeMenu(parentId) {
	...
	function (xhrObj) {
		...
		$("#RightTree").tree({
			...
			//    data = data.Data
			//},
			loadFilter: function (xhrObj) {
				if (msgProcess(xhrObj) == false) return false;
				return xhrObj.Data;
			},
			onBeforeExpand: function (node, param) {
...
function SignOut() {
	...
	$.messager.confirm(index_lang_tip, index_YouWantToExitTheSystem, function (r) {
		if (r) {
			$.post("/Account/LogOut", function (xhrObj) {
