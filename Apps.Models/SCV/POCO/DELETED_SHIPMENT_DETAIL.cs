//------------------------------------------------------------------------------
// <auto-generated>
//    此代码是根据模板生成的。
//
//    手动更改此文件可能会导致应用程序中发生异常行为。
//    如果重新生成代码，则将覆盖对此文件的手动更改。
// </auto-generated>
//------------------------------------------------------------------------------

namespace Apps.Models
{
	public partial class DELETED_SHIPMENT_DETAIL
	{
		public Apps.Models.SCV.DELETED.DELETED_SHIPMENT_DETAIL_MODEL ToPOCO(){
			return new Apps.Models.SCV.DELETED.DELETED_SHIPMENT_DETAIL_MODEL(){
								INTERNAL_SHIPMENT_LINE_NUM=this.INTERNAL_SHIPMENT_LINE_NUM,
				INTERNAL_SHIPMENT_NUM=this.INTERNAL_SHIPMENT_NUM,
				WAREHOUSE=this.WAREHOUSE,
				COMPANY=this.COMPANY,
				SHIPMENT_ID=this.SHIPMENT_ID,
				ERP_ORDER=this.ERP_ORDER,
				ERP_ORDER_LINE_NUM=this.ERP_ORDER_LINE_NUM,
				SHIPMENT_TYPE=this.SHIPMENT_TYPE,
				ITEM=this.ITEM,
				ITEM_DESC=this.ITEM_DESC,
				TOTAL_QTY=this.TOTAL_QTY,
				REQUEST_QTY=this.REQUEST_QTY,
				QUANTITY_UM=this.QUANTITY_UM,
				ALLOCATION_RULE=this.ALLOCATION_RULE,
				PICK_LOC=this.PICK_LOC,
				USER_STAMP=this.USER_STAMP,
				DATE_TIME_STAMP=this.DATE_TIME_STAMP,
				ATTRIBUTE_TRACK=this.ATTRIBUTE_TRACK,
				ATTRIBUTE1=this.ATTRIBUTE1,
				ATTRIBUTE2=this.ATTRIBUTE2,
				ATTRIBUTE3=this.ATTRIBUTE3,
				ATTRIBUTE4=this.ATTRIBUTE4,
				ATTRIBUTE5=this.ATTRIBUTE5,
				ATTRIBUTE6=this.ATTRIBUTE6,
				ATTRIBUTE7=this.ATTRIBUTE7,
				ATTRIBUTE8=this.ATTRIBUTE8,
				INVENTORY_STS=this.INVENTORY_STS,
				DOCK_LOC=this.DOCK_LOC,
				PACKING_CLASS=this.PACKING_CLASS,
				STATUS=this.STATUS,
				INTERNAL_WAVE_NUM=this.INTERNAL_WAVE_NUM,
				TOTAL_WEIGHT=this.TOTAL_WEIGHT,
				WEIGHT_UM=this.WEIGHT_UM,
				TOTAL_VOLUME=this.TOTAL_VOLUME,
				VOLUME_UM=this.VOLUME_UM,
				BACK_ORDER_LINE_NUM=this.BACK_ORDER_LINE_NUM,
				USER_DEF1=this.USER_DEF1,
				USER_DEF2=this.USER_DEF2,
				USER_DEF3=this.USER_DEF3,
				USER_DEF4=this.USER_DEF4,
				USER_DEF5=this.USER_DEF5,
				USER_DEF6=this.USER_DEF6,
				USER_DEF7=this.USER_DEF7,
				USER_DEF8=this.USER_DEF8,
				ITEM_LIST_PRICE=this.ITEM_LIST_PRICE,
				ITEM_NET_PRICE=this.ITEM_NET_PRICE,
				SERIAL_NUM_TRACK=this.SERIAL_NUM_TRACK,
				USER_DEF9=this.USER_DEF9,
				USER_DEF10=this.USER_DEF10,
				ORIGINAL_ORDER_NUM=this.ORIGINAL_ORDER_NUM,
				CREATE_DATE_TIME=this.CREATE_DATE_TIME,
				CREATE_USER=this.CREATE_USER,
				HOST_ITEM=this.HOST_ITEM,
				UPLOAD_INTERFACE_REQUIRED=this.UPLOAD_INTERFACE_REQUIRED,
			};
		}
	}
}
